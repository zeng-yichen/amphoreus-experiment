...
try:
    import vortex as P
    MEMORY_ROOT      = P.MEMORY_ROOT          # pathlib.Path("memory")
    _AUTH_CSV_PATH   = P.ordinal_auth_csv()   # memory/ordinal_auth_rows.csv
except ImportError:
...
HTML_MODEL       = "claude-sonnet-4-6"   # cheaper model for markdown→HTML conversion
...
def _load_client_files(client_name: str) -> dict[str, str]:
    ...
    for directory in (client_dir / "transcripts", client_dir):
        ...
        for pattern in ("*.txt", "*.md", "*.pdf", "*.docx"):
            for fp in sorted(directory.glob(pattern)):
                ...
                if rel not in results:
                    try:
                        results[rel] = fp.read_text(encoding="utf-8", errors="replace")
                    except Exception as exc:
