...
from backend.src.agents.cyrene import (
    ...
    _TOOLS as _CYRENE_TOOL_SCHEMAS,
)
...
def _register(name: str, handler_fn):
    """Pull the schema from Cyrene's _TOOLS list so the server stays in sync."""
...
_register("query_observations", _wrap(_query_observations))
_register("query_top_engagers", _wrap(_query_top_engagers))
...
_register("execute_python", _wrap(_execute_python))
_register("search_linkedin_corpus", _wrap(_search_linkedin_corpus))
_register("web_search", _wrap(_web_search))
_register("fetch_url", _wrap(_fetch_url))
