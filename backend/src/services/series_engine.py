...
Usage:
    ...
    # Plan a new series:
    series = plan_series("hensley-biostats", "sample size estimation", num_posts=5)

    # Before Stelle generates:
    context = get_stelle_series_context("hensley-biostats")
    ...
    check_series_health("hensley-biostats")
"""
...
You are planning a multi-post LinkedIn content series for a B2B thought leader. \
The series should build compound engagement — each post references and builds on \
the previous ones, creating a narrative arc that rewards return readers.

LinkedIn's algorithm rewards topic consistency. A 4-6 post series on one theme \
builds exponentially more authority than the same number of disconnected posts.

{arc_guidance}

Each post outline should:
- Be 1-2 sentences describing the specific angle and key point
- Reference the specific transcript source (file + approximate timestamp)
- Build on what came before — post 3 should reference concepts from posts 1-2

Respond with ONLY a JSON array:
[
  {{
    "position": 1,
    "arc_role": "first role from the arc",
    "outline": "Introduce the problem of X. Open with the moment from transcript where Y happened.",
    "transcript_source": "Transcript 2 [14:30-18:00]"
  }},
  ...
]
"""
