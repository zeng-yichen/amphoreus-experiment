...
Usage:
    ...
    schedule = compute_optimal_schedule("hensley-biostats", num_posts=6)
    # → [datetime(...), datetime(...), ...]  each with optimal day+hour

    # For single post:
    slot = recommend_next_slot("hensley-biostats")
    # → datetime(2026, 4, 5, 14, 0) — next Thursday at 14:00 UTC

    # For Stelle context injection:
    context = build_scheduling_context("hensley-biostats")
