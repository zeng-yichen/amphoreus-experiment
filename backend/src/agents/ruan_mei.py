...
to Stelle via the `query_observations` tool at generation time.

Retired from RuanMei's responsibilities:
  - `recommend_context()`: pre-chewed prose context for Stelle. Stelle
    now reads raw observations via `query_observations`.
...
class RuanMei:
    ...
    def _apply_update(
        ...
        # and query_observations (they filter on scored/finalized).
    ...
    # directly via `query_observations` / `execute_python` tools.
    # -------------------------------------------------------------
