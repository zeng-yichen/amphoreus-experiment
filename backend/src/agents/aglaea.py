...
def _run_direct_fallback(company_keyword: str, client_name: str) -> str | None:
    ...
    # Analyst findings — same injection as the Pi path.
    analyst_ctx = ""
    try:
        import json as _json_aglaea_fb
        _findings_path = P.memory_dir(company_keyword) / "analyst_findings.json"
        if _findings_path.exists():
            _af = _json_aglaea_fb.loads(_findings_path.read_text(encoding="utf-8"))
            _findings = _af.get("findings", [])
            if _findings:
                _lines = ["## Engagement Analysis\n"]
                for _f in _findings:
                    _lines.append(f"- **[{_f.get('confidence', '?')}]** {_f.get('claim', '')}")
                analyst_ctx = "\n".join(_lines)
    except Exception:
        pass
...
def generate_briefing(client_name: str, company_keyword: str, event_callback=None) -> str:
    ...
    # Inject analyst findings as context. The analyst agent forms and tests
    # hypotheses about what predicts engagement using statistical tools +
    # LinkedIn-wide data. Its findings are raw data with confidence levels —
    # the LLM reads them and decides which interview questions to prioritize.
    try:
        import json as _json_aglaea
        _findings_path = P.memory_dir(company_keyword) / "analyst_findings.json"
        if _findings_path.exists():
            _af = _json_aglaea.loads(_findings_path.read_text(encoding="utf-8"))
            _findings = _af.get("findings", [])
            if _findings:
                _ctx_lines = [
                    "## Engagement Analysis (auto-generated by analyst agent)\n",
                    "The following findings were discovered by statistically analyzing "
                    "this client's engagement history. Use them to inform which "
                    "interview questions you prioritize — dig for more stories in "
                    "areas that drive engagement. The data is directional, not definitive.\n",
                ]
                for _f in _findings:
                    _conf = _f.get("confidence", "suggestive")
                    _claim = _f.get("claim", "")
                    _ctx_lines.append(f"- **[{_conf}]** {_claim}")
                _strategy_ctx = "\n".join(_ctx_lines)
                user_prompt += "\n\n---\n\n" + _strategy_ctx
                logger.info("[Aglaea] Injected analyst findings (%d findings)",
                            len(_findings))
    except Exception as _e:
        logger.debug("[Aglaea] Analyst findings skipped: %s", _e)
