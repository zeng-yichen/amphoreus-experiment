...
def distill_directives(company: str, force: bool = False) -> Optional[list[dict]]:
    ...
    # Preserve first_computed_at across refreshes — the stable anchor for
    # the active_directives backfill. computed_at moves forward on every
    # refresh; first_computed_at stays at the original distillation time.
    _prior_first = None
    if cache_path.exists():
        try:
            _prior = json.loads(cache_path.read_text(encoding="utf-8"))
            _prior_first = _prior.get("first_computed_at")
        except Exception:
            pass
    ...
    cache_data = {
        "directives": valid,
        "computed_at": now_iso,
        "first_computed_at": _prior_first or now_iso,
        "signal_counts": {
...
def backfill_active_directives(company: str, rm_state: dict, directives: list[dict]) -> int:
    ...
    Stamps directive IDs on ALL scored observations that don't already have
    active_directives populated. No timestamp comparison — the directives
    represent the client's editorial preferences which are timeless ("use
    clinical operations not ClinOps" was always their preference, not just
    since the distiller first ran).

    For forward attribution (new posts generated by Stelle), active_directives
    is stamped at generation time with the exact set in the prompt. This
    backfill handles historical observations that predate the tracking.
